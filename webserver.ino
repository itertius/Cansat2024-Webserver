...
const sensorElements = {
  ...
  Press: document.getElementById("BMP280Press"),
  MCPTemp: document.getElementById("MCP9808Temp"),
  Lat: document.getElementById("GPSLat"),
  Lng: document.getElementById("GPSLng"),
  GPSAltitude: document.getElementById("GPSAltitude"),
  AccX: document.getElementById("GY521AccX"),
  AccY: document.getElementById("GY521AccY"),
  AccZ: document.getElementById("GY521AccZ"),
  GyX: document.getElementById("GY521GyX"),
  GyY: document.getElementById("GY521GyY"),
  GyZ: document.getElementById("GY521GyZ"),
};
...
String GPSLat = "/Lat";
String GPSLng = "/Lng";
String GPSAltitude = "/GPSAltitude";
String MCPTemp = "/MCPTemp";
...
void loop() {
  ...
  if (LoRa.available()) {
    ...
    if (Module == "BMP280") {
      SyncSec = getValue(incoming, ',', 1);
      Press = getValue(incoming, ',', 2);

      JSONtxt = "{\"SyncSec\":\"" + valueSyncSec + "\",";
      JSONtxt += "\"Press\":\"" + valuePress + "\"}";

      webSocket.broadcastTXT(JSONtxt);
    }
    else if (Module == "GY521") {
      SyncSec = getValue(incoming, ',', 1);
      AccX = getValue(incoming, ',', 2);
      AccY = getValue(incoming, ',', 3);
      AccZ = getValue(incoming, ',', 4);
      GyX = getValue(incoming, ',', 5);
      GyY = getValue(incoming, ',', 6);
      GyZ = getValue(incoming, ',', 7);

      JSONtxt = "{\"SyncSec\":\"" + SyncSec + "\",";
      JSONtxt += "\"AccX\":\"" + AccX + "\",";
      JSONtxt += "\"AccY\":\"" + AccY + "\",";
      JSONtxt += "\"AccZ\":\"" + AccZ + "\",";
      JSONtxt += "\"GyX\":\"" + GyX + "\",";
      ...
      SyncSec = getValue(incoming, ',', 1);
      MCPTemp = getValue(incoming, ',', 2);
      
      JSONtxt = "{\"SyncSec\":\"" + SyncSec + "\",";
      JSONtxt += "\"MCPTemp\":\"" + MCPTemp + "\"}";

      webSocket.broadcastTXT(JSONtxt);
    }
    else if (Module == "GPS") {
      SyncSec = getValue(incoming, ',', 1);
      Lat = getValue(incoming, ',', 2);
      Lng = getValue(incoming, ',', 3);
      GPSAltitude = getValue(incoming, ',', 4);

      JSONtxt = "{\"SyncSec\":\"" + SyncSec + "\",";
      JSONtxt += "\"Lat\":\"" + Lat + "\",";
      JSONtxt += "\"Lng\":\"" + Lng + "\",";
      JSONtxt += "\"GPSAltitude\":\"" + GPSAltitude + "\"}";
    ...
    if (Firebase.ready() && signupOK) {
      if (Module == "BMP280" || Module == "GY521" || Module == "MCP9808" || Module == "GPS") {
        if (currentMillis - sendDataPrevMillis > 5000 || sendDataPrevMillis == 0) { // set store freq
          ...
          json.clear();
          if (Module == "BMP280") {
            json.set(BMPPressure.c_str(), Press);
          }
          else if (Module == "GY521") {
            json.set(GYAccX.c_str(), AccX);
            json.set(GYAccY.c_str(), AccY);
            json.set(GYAccZ.c_str(), AccZ);
            json.set(GYX.c_str(), GyX);
            json.set(GYY.c_str(), GyY);
            json.set(GYZ.c_str(), GyZ);
          }
          else if (Module == "MCP9808") {
            json.set(MCPTemp.c_str(), MCPTemp);
          }
          else if (Module == "GPS") {
            json.set(GPSLat.c_str(), Lat);
            json.set(GPSLng.c_str(), Lng);
            json.set(GPSAltitude.c_str(), GPSAltitude);
          }
